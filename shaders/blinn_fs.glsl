...
/**
 Power of clamp(dot(viewDirection, r), 0.0, 1.0) used in Blinn specular
 reflection model
 */
#define GAMMA 50.0
...
/**
 ...
 as a sum of Lambert diffuse reflection and Blinn specular reflection
 */
vec3 lambertBlinn(vec3 mD, vec3 nNormal, vec3 viewDirection, vec3 lightDirNorm) {
    ...
    vec3 fSpecular = clamp(mS*pow(clamp(dot(nNormal, halfVector), 0.0, 1.0), GAMMA), 0.0, 1.0);
    vec3 lambertBlinnLight = fDiffuse+fSpecular;
    return lambertBlinnLight;
}
...
/**
 ...
 The BRDF model that is used is the Lambert diffuse reflection and the Blinn spcular reflection.
 */
void main() {
    ...
    if(isNight) {
        ...
        vec3 fpl1 = lambertBlinn(mD, nNormal, viewDirection, lightDirNorm);
        ...
        vec3 fpl2 = lambertBlinn(mD, nNormal, viewDirection, lightDirNorm);
        ...
        vec3 fSpot = lambertBlinn(mD, nNormal, viewDirection, lightDirNorm);
    ...
    vec3 fDirectional = lambertBlinn(mD, nNormal, viewDirection, lightDirNorm);
